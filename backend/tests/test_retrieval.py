...
class TestFindRelevantChunks:
    def test_returns_chunks_ordered_by_similarity_to_the_query(self, db_session):
        ...
        close = _add_chunk(db_session, document, "close match", [1.0, 0.0, 0.0])
        far = _add_chunk(db_session, document, "far match", [0.0, 1.0, 0.0])
        _add_chunk(db_session, document, "opposite match", [-1.0, 0.0, 0.0])

        results = find_relevant_chunks([0.9, 0.1, 0.0], org.id, db_session, limit=2)
    ...
    def test_never_returns_chunks_from_another_org(self, db_session):
        ...
        _add_chunk(db_session, doc_a, "org a content", [1.0, 0.0, 0.0])
        org_b_chunk = _add_chunk(db_session, doc_b, "org b content", [1.0, 0.0, 0.0])

        results = find_relevant_chunks([1.0, 0.0, 0.0], org_b.id, db_session, limit=10)
    ...
    def test_respects_the_limit(self, db_session):
        org, document = _make_org_with_document(db_session)
        for i in range(5):
            _add_chunk(db_session, document, f"chunk {i}", [1.0, 0.0, 0.0])

        results = find_relevant_chunks([1.0, 0.0, 0.0], org.id, db_session, limit=3)
